...
vec2 twistCoords(vec2 warpedCoords) {
    float twistAmount = 0.3;
    float sinVal = sin(time_f * twistAmount);
    float cosVal = cos(time_f * twistAmount);
    return vec2(
        warpedCoords.x * cosVal - warpedCoords.y * sinVal,
        warpedCoords.x * sinVal + warpedCoords.y * cosVal
    );
}
...
void main(void) {
    ...
    warpedCoords = twistCoords(warpedCoords);
    color = texture(textTexture, warpedCoords);
}