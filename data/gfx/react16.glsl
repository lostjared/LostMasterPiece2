...
void main(void) {
    ...
    vec4 color1 = texture(textTexture, tcAdjusted);
    ...
    vec4 color2 = texture(textTexture, stretchedCoord);

    float glitchFactor = sin(time_f * 50.0);
    vec4 glitchColor = texture(textTexture, tc + vec2(glitchFactor * 0.1, glitchFactor * 0.1));
