...
mat3 getCameraMatrix(float time) {
    // Compute zoom in/out
    float zoom = 0.5 + 0.5 * sin(time * 0.5);
    float angleY = time; // Rotation along Y-axis
...
void main(void) {
    ...
    mat3 camera = getCameraMatrix(time_f);
    vec2 transformedUV = (camera * vec3(tc * 2.0 - 1.0, 1.0)).xy * 0.5 + 0.5;

    vec4 pix = blur(textTexture, transformedUV, iResolution);
    pix = pix * time_t;
    pix = colorShift(pix);
    ...
    pix.rgb = mix(pix.rgb, purple, 0.3); // Adjust the 0.3 value to control the intensity of the tint.
