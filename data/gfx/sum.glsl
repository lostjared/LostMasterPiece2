...
void main() {
    ...
    vec4 sum = texture(textTexture, tc) * 4.0;
    sum += texture(textTexture, tc + vec2(texelSize.x, 0.0));
    sum += texture(textTexture, tc - vec2(texelSize.x, 0.0));
    sum += texture(textTexture, tc + vec2(0.0, texelSize.y));
    sum += texture(textTexture, tc - vec2(0.0, texelSize.y));
    sum /= 8.0;

    float time_t = pingPong(time_f, 10.0) + 2.0;
    float time_x = pingPong(time_f, 5.0) + 2.0;
    vec4 b_tex = blur(textTexture, tc, iResolution);
