...
// Texture textTexturelers
uniform sampler2D textTexture;   // Base texture
uniform sampler2D textTexture1;  // Texture #1
uniform sampler2D textTexture2;  // Texture #2
uniform sampler2D textTexture3;  // Texture #3
uniform sampler2D textTexture4;  // Texture #4
...
void main(void)
{
    ...
    //--------------------------------------------------//
    {
        ...
        // textTexturele from textTexture1 with the new swirl-based coords
        vec4 col1 = texture(textTexture1, swirlUV);
    ...
    vec4 colorFractal2;
    {
        ...
        float fractVal = fractalJulia(fractalUV, 
                                       ...
                                       16);
        // Use fractVal to modulate textTextureling coordinates
        // E.g., add fractVal to both x, y
        vec2 newUV = (uv + fractVal * 0.3) * 0.5 + 0.5;
        // textTexturele from textTexture2
        colorFractal2 = texture(textTexture2, newUV);
    ...
    vec4 colorKaleido3;
    {
        ...
        colorKaleido3 = texture(textTexture3, kaleidoUV);
        // Boost saturation
    ...
    vec4 colorFractal4;
    {
        ...
        colorFractal4 = texture(textTexture4, swirl4UV);
        // Darken or lighten based on fractVal2
        colorFractal4.rgb *= mix(0.5, 1.5, fractVal2);
    }

    //--------------------------------------------------//
    // 5. Base texture (textTexture)
    //--------------------------------------------------//
    vec4 baseColor = texture(textTexture, tc);
    ...
    vec4 colorSwirl    = texture(textTexture1, swirl(uv, vec2(0.0), 1.0, sin(time_f) * 5.0) * 0.5 + 0.5);
