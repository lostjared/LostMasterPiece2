...
void main(){
    ...
    // gooey cloud warp
    float cd = fbm(l2*3.0 - time_f*0.5) - 0.5; 
    vec2 dir = normalize(l2 - ctr);
    vec2 cloudWarp = dir * cd * 0.15;
    vec2 u3 = l2 + cloudWarp;

    vec2 nOff  = rotate2d(u3 + vec2(noise(u3*10.0+time_f)*0.02), time_f*0.3);
    vec2 final = mix(nOff, uv, 0.02);

    vec4 orig    = texture(textTexture, uv);
    vec4 warped  = texture(textTexture, final);
    vec4 blur    = radialBlur(final, 10);
    vec3 clouds  = cloudLayer(uv*1.8);
    vec3 rays    = cosmicRays(uv, ctr)*0.8;
    vec3 grad    = rainbow(time_f*4.0 + length(uv-ctr)*6.0);
    vec3 mix1    = mix(warped.rgb, blur.rgb, 0.6);
    vec3 mix2    = mix(mix1, clouds, 0.5);
    vec3 mix3    = mix(mix2, rays, 0.7);
    float b      = pow(smoothstep(0.0,1.0,length(uv-ctr)),0.5);
    vec3 finalC  = mix(orig.rgb, mix3+grad*0.4, b);

    color = vec4(finalC,1.0);
}
