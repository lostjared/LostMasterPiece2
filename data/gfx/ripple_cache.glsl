...
uniform sampler2D textTexture;   // current frame
uniform sampler2D textTexture1;  // older frame #1
uniform sampler2D textTexture2;  // older frame #2
uniform sampler2D textTexture3;  // older frame #3
uniform sampler2D textTexture4;  // older frame #4
...
void main(void)
{
    ...
    // 4) textTexturele from your 4 older frames with slightly shifted coords
    //    to produce a ghostly "delayed" or "rippled" effect
    vec4 f1 = texture(textTexture1, rippleUV + glitchOff * 0.5);
    vec4 f2 = texture(textTexture2, rippleUV + glitchOff * 1.0);
    vec4 f3 = texture(textTexture3, rippleUV - glitchOff * 0.5);
    vec4 f4 = texture(textTexture4, rippleUV - glitchOff * 1.0);
