...
void main(void) {
    ...
    // textTexturele texture with psychedelic combination
    vec3 c = vec3(0.0);
    c.r += texture(textTexture, tc1).r * 1.2;
    c.b += texture(textTexture, tc2).b * 1.2;
    c.rb += texture(textTexture, tc3).rb * 0.8;

    // Apply purple tint and boost intensity
    color = vec4(c * purpleTint * 1.5, 1.0);
    color = mix(color, texture(textTexture, tc), 0.5);
    color = vec4(color.rgb, texture(textTexture, tc).a);
}