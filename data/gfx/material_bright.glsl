...
void main(void)
{
    if(restore_black_value == 1.0 && texture(textTexture, tc) == vec4(0, 0, 0, 1))
        discard;
    color = texture(textTexture, tc) * texture(mat_textTexture, tc);
    float x = gl_FragCoord.x;
    float y = gl_FragCoord.y;
    vec2 cord1 = vec2(tc[0]/x, tc[1]/y);
    vec2 cord2 = vec2(tc[0]/x+32, tc[1]/y+16);
    vec2 cord3 = vec2(tc[0]/x+64, tc[1]/y+32);
    vec4 col1 = texture(textTexture, cord1) * texture(mat_textTexture, cord1);
    vec4 col2 = texture(textTexture, cord2) * texture(mat_textTexture, cord2);
    vec4 col3 = texture(textTexture, cord3) * texture(mat_textTexture, cord3);
