...
uniform sampler2D textTexture;      // The texture we're textTextureling
...
void main(void)
{
    ...
    // Convert back to [0,1] range for textTextureling
    uv.x /= aspect;
    uv = uv * 0.5 + 0.5;
    ...
    // Blend them into the textTextureled texture
    baseColor.r += pulseR;
    baseColor.g += pulseG;
    baseColor.b += pulseB;
    ...
    baseColor *= texture(textTexture, uv);
