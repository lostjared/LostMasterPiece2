...
void main(void) {
    ...
    // textTexturele texture with different displacements for each channel
    vec4 originalColor = texture(textTexture, tc);
    originalColor.r = texture(textTexture, rippleTC_R).r;
    originalColor.g = texture(textTexture, rippleTC_G).g;
    originalColor.b = texture(textTexture, rippleTC_B).b;
