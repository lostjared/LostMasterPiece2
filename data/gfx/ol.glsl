...
uniform sampler2D textTexture;         // Texture textTextureler
uniform vec2 iResolution;      // Screen resolution
uniform vec4 iMouse;           // Mouse position

// Parameters to control the glitch effect
uniform float frequency = 0.5f; // Main frequency of the warp
uniform float strength = 1.0f;   // Intensity of the warp
...
mousePos.y) * strength;
    ...
    color = mix(texture(textTexture, sin(finalTexCoord * time_f)), texture(textTexture, tc), 0.5);
}