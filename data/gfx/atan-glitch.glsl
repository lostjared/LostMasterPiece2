...
void main(void) {
    ...
    vec4 shiftedColor = vec4(
        texture(textTexture, rotatedTC + glitchOffset + vec2(rgbShift, 0.0)).r,
        texture(textTexture, rotatedTC + glitchOffset).g,
        texture(textTexture, rotatedTC + glitchOffset - vec2(rgbShift, 0.0)).b,
        1.0
    );
    ...
    color = mix(color, texture(textTexture, warpedCoords), 0.7);
    ...
    color.rg = texture(textTexture, warpedCoords + vec2(blockGlitch)).rg;
}