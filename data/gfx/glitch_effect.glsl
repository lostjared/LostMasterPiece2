...
void main(void)
{
    ...
    if(restore_black_value == 1.0 && texture(textTexture, tcOffset) == vec4(0, 0, 0, 1))
        discard;
    
    vec4 baseColor = texture(textTexture, tcOffset);
    vec4 glitchColor = texture(textTexture, tc + glitchOffset * 2.0);
