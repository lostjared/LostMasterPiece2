...
uniform sampler2D textTexture1; // cache texture 0
uniform sampler2D textTexture2; // cache texture 1
...
void main(void) {
    ...
    // textTexturele textures with modulation
    vec4 tex1 = texture(textTexture1, distortedTC);
    vec4 tex2 = texture(textTexture2, distortedTC * 1.2);
    vec4 tex3 = texture(textTexture3, tc * 0.8 + tex1.xy * 0.05);
    vec4 tex4 = texture(textTexture4, tc * 1.1 - tex2.xy * 0.03);
    vec4 baseTex = texture(textTexture, distortedTC);
