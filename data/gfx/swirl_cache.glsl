...
uniform sampler2D textTexture;   // base
uniform sampler2D textTexture1;  // layer 1
uniform sampler2D textTexture2;  // layer 2
uniform sampler2D textTexture3;  // layer 3
uniform sampler2D textTexture4;  // layer 4
...
void main(void)
{
    ...
    // Distorted UVs for each textTextureler
    ...
    // textTexturele from each texture with the distorted coordinates
    vec4 base   = texture(textTexture,  uv0);
    vec4 one    = texture(textTexture1, uv1);
    vec4 two    = texture(textTexture2, uv2);
    vec4 three  = texture(textTexture3, uv3);
    vec4 four   = texture(textTexture4, uv4);
